...
public class KontrolaResource {
  ...
  @APIResponses(value = {
      ...
      @APIResponse(responseCode = "500", description = "Greška na poslužitelju")})
  @Counted(name = "kontrolaKorisnikDodajCount",
      description = "Broj poziva korisnik/dodaj - POST")
  @Timed(name = "kontrolaKorisnikDodajTime",
      description = "Vrijeme izvršavanja korisnik/dodaj - POST")
  @Timeout(3000)
  @Retry(maxRetries = 3)
  @Fallback(fallbackMethod = "metodaKodPogreskeKorisnikDodaj")
  public Response korisnikDodaj(
      @PathParam("korisnik") String korisnik) {
    ...
    if (odgovor.startsWith("OK")) {
      return Response.status(Response.Status.CREATED)
          .entity(oKorisnik).type(MediaType.APPLICATION_JSON).build();
    }
  ...
  public Response korisnikProvjeri(@PathParam("korisnik") String korisnik) {
    ...
    if (odgovor.startsWith("OK")) {
      return Response.ok().entity(oKorisnik).type(MediaType.APPLICATION_JSON).build();
    }
    if (odgovor.startsWith("ERROR 12")) {
      return Response.status(Response.Status.NOT_FOUND)
          .entity(new Korisnik(null, null, null, null, oKorisnik.email()))
  ...
  public Response ulaznicaDodaj(@PathParam("korisnik") String korisnik) {
    ...
    if (odgovor.startsWith("OK")) {
      String id = odgovor.trim().split("\\s+")[1];
      Ulaznica oUlaznica = new Ulaznica(id, oKorisnik.email(), null, null);
      return Response.status(Response.Status.CREATED)
          .entity(oUlaznica).type(MediaType.APPLICATION_JSON).build();
    }
    if (odgovor.startsWith("ERROR 12")) {
      return Response.status(Response.Status.NOT_FOUND)
          .entity(new Ulaznica(null, oKorisnik.email(), null, null))
          .type(MediaType.APPLICATION_JSON).build();
    }
  ...
  public Response ulaznicaProvjeri(@PathParam("id") String id) {
    ...
    if (odgovor.startsWith("OK")) {
      Ulaznica oUlaznica = new Ulaznica(id, null, null, null);
      return Response.ok().entity(oUlaznica).type(MediaType.APPLICATION_JSON).build();
    }
    if (odgovor.startsWith("ERROR 14")) {
      return Response.status(Response.Status.NOT_FOUND)
          .entity(new Ulaznica(id, null, null, null))
          .type(MediaType.APPLICATION_JSON).build();
    }
    return Response.status(Response.Status.INTERNAL_SERVER_ERROR).build();
  }
  ...
  public Response ulaznicaKorisnik(@PathParam("id") String id) {
    ...
    if (odgovor.startsWith("OK")) {
      String email = odgovor.substring(2).trim();
      Korisnik oKorisnik = new Korisnik(null, null, null, null, email);
      return Response.ok().entity(oKorisnik).type(MediaType.APPLICATION_JSON).build();
    }
    if (odgovor.startsWith("ERROR 14")) {
      return Response.status(Response.Status.NOT_FOUND)
          .entity(new Korisnik(null, null, null, null, null))
          .type(MediaType.APPLICATION_JSON).build();
    }
    return Response.status(Response.Status.INTERNAL_SERVER_ERROR).build();
  }
  ...
  public Response adminUlaznicaPonisti() {
    ...
    if (odgovor.startsWith("OK")) {
      return Response.ok().entity(odgovor)
          .type(MediaType.APPLICATION_JSON).build();
    }
    if (odgovor.startsWith("ERROR 14")) {
      return Response.status(Response.Status.NOT_FOUND).build();
  ...
  public Response metodaKodPogreskeKorisnikDodaj(
      @PathParam("korisnik") String korisnik) {
    return Response.status(Status.SERVICE_UNAVAILABLE).build();
  }
