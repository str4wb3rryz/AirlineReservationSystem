...
public class KorisniciResource {
  ...
  public Response dajSveKorisnike(
      @QueryParam("odStranice") Integer odStranice,
      @QueryParam("stranica") Integer stranica) {
    try (var vezaBP = Main.dajVezu()) {
      var korisnikDAO = new KorisnikDAO(vezaBP);
      List<Korisnik> korisnici;
      if (odStranice != null && stranica != null) {
        // odStranice je redni broj stranice (počinje od 1); DAO očekuje pomak (offset) redaka.
        int offset = Math.max(0, (odStranice - 1) * stranica);
        korisnici = korisnikDAO.dohvatiSveStranicenje(offset, stranica);
      } else {
        korisnici = korisnikDAO.dohvatiSve();
      }
      if (korisnici != null) {
