...
import edu.unizg.foi.nwtis.strawberryz.vjezba_08_dz_3.jpa.entiteti.Grupe;
...
public class KorisniciFacade extends EntityManagerProducer implements Serializable {
  ...
  public void create(Korisnici korisnici, String grupa) {
    var em = getEntityManager();
    em.persist(korisnici);
    if (grupa != null && !grupa.isBlank()) {
      Grupe g = em.find(Grupe.class, grupa);
      if (g != null) {
        if (korisnici.getGrupes() == null) {
          korisnici.setGrupes(new ArrayList<>());
        }
        korisnici.getGrupes().add(g);
      }
    }
  }
