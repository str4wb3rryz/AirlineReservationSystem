...
<head>
  ...
  <title>Pregled računa zastupnika</title>
...
<body>
  <h2>Pregled računa zastupnika</h2>
  <c:if test="${zastupnik != null}">
    <p class="korisnik-traka">Prijavljen je zastupnik: <strong>${zastupnik}</strong></p>
  </c:if>

  <p>
    <c:choose>
      <c:when test="${mozeOtvoritiNovi}">
        <a href="${pageContext.servletContext.contextPath}/mvc/aviotvrtke/otvoriRacun">Otvori novi račun za kupovinu avio karata</a>
      </c:when>
      <c:otherwise>
    ...
    <tbody>
      <c:forEach var="r" items="${racuni}">
        <tr>
          ...
          <td>
            <a href="${pageContext.servletContext.contextPath}/mvc/aviotvrtke/pregledStavkiRacuna/${r.id}">Pregled stavki</a>
          </td>
          <td>
            <c:choose>
              <c:when test="${r.otvoren and r.uskladjen}">
                <a href="${pageContext.servletContext.contextPath}/mvc/aviotvrtke/zatvoriRacun/${r.id}">Zatvori račun</a>
              </c:when>
